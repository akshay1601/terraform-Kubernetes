...
resource "aws_iam_policy" "clients-api" {
  name = "${replace(local.name_suffix_name, "-", "")}-ResourceApiPolicy"
    ...
    "Statement": [
        {
            ...
            "Resource": [
                "arn:aws:dynamodb:${(local.name_suffix_region)}:${replace(local.name_suffix_account, "-", "")}:table/${replace(local.name_suffix_name, "-", "")}-resources",
